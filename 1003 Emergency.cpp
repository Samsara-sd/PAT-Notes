...
int find_nearest_city(int city_num, const vector<int>& dist, const vector<bool>& sure)
{
    int ans = -1;
...
void Dijkstra(int city_num, int source, int destination, vector<vector<int>>& graph, vector<int>& team_num, vector<int>& dist, vector<int>& count, vector<bool>& sure, vector<int>& pre)
{
    ...
    while( this_city != -1 )
    {
        ...
        if( pre[this_city] != -1 )
        {
            team_num[this_city] += team_num[pre[this_city]];
        }
        for (int iter = 0; iter < city_num; iter++)
        {
            if( !sure[iter] && graph[this_city][iter] != infinity )
            {
                if( dist[this_city] + graph[this_city][iter] < dist[iter] )
                {
                    pre[iter] = this_city;
                    dist[iter] = dist[this_city] + graph[this_city][iter];
                    count[iter] = count[this_city];
                }
                else if( dist[this_city] + graph[this_city][iter] == dist[iter] )
                {
...
int main(int argc, const char* argv[])
{
    ...
    for (int iter = 1; iter <= city_num; iter++)
    {
        vector<int> temp_row;
    ...
    for (int iter = 1; iter <= road_num; iter++)
    {
